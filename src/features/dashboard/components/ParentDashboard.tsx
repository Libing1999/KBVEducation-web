...
export function ParentDashboard() {
  ...
  if (isLoading) return <LoadingState label="Loading your child’s progress…" />;
  if (isError || !progress) {
    return (
      <ErrorState
        message="We couldn’t load your child’s progress. They may not be linked to your account yet."
        onRetry={() => refetch()}
      />
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-800">{progress.studentName}’s Progress</h1>
        <p className="text-sm text-slate-500">A read-only view of your child’s activity.</p>
      </div>
